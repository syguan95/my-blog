...
module.exports = async (ctx, next) => {
  if (ctx.request.url.indexOf("register") > -1 || ctx.request.url.indexOf("login") > -1 || ctx.request.url.indexOf("autoLogin") > -1 || ctx.request.url.indexOf("article") > -1 && ctx.request.method === "GET") {  //注册接口、登录接口、自动登录接口不需要验证权限
    await next();
  } else {
